    ...
    <section className="pricing-admin" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 140px)" }}>
      <div className="pricing-admin__head" style={{ marginBottom: 16 }}>
        ...
        <div className="pricing-admin__head-actions">
          <Button onClick={saveAll} disabled={!dirtyIds.length || saving !== null} style={{ background: "#f49921", color: "#000", fontWeight: "bold" }}>
            <Save size={18} style={{ marginRight: 6 }} /> 
            {saving === "all" ? "جارٍ الحفظ..." : `حفظ التعديلات (${dirtyIds.length})`}
          </Button>
        </div>
      </div>
